function handleSearch(event) {
    ...
    fetch(`/jobs/search?company=${encodeURIComponent(company)}&category=${encodeURIComponent(category)}&location=${encodeURIComponent(location)}`)
        ...
        .then(data => {
            ...
            // Create HTML for jobs - with sanitization to prevent XSS
                    ...
                    <div class="row g-4">
                        <div class="col-sm-12 col-md-8 d-flex align-items-center">
                            <img class="flex-shrink-0 img-fluid border rounded" 
                                src="/static/${escapeHTML(job.company_logo || '')}" 
                                alt="${escapeHTML(job.company || '')} logo"
                                style="width: 80px; height: 80px; object-fit: cover;">
                            <div class="text-start ps-4">
                                <h5 class="mb-3"><a href="/jobs/${encodeURIComponent(job.id)}" class="text-dark">${escapeHTML(job.title || '')}</a></h5>
                                <span class="text-truncate me-3"><i class="fa fa-building text-primary me-2"></i>${escapeHTML(job.company || '')}</span>
                                <span class="text-truncate me-3"><i class="fa fa-map-marker-alt text-primary me-2"></i>${escapeHTML(job.location || '')}</span>
                                <span class="text-truncate me-0"><i class="far fa-money-bill-alt text-primary me-2"></i>${escapeHTML(job.salary || 'Not specified')}</span>
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-4 d-flex flex-column align-items-start align-items-md-end justify-content-center">
                            <div class="d-flex mb-3">
                                <a class="btn btn-primary" href="/jobs/${encodeURIComponent(job.id)}">View Details</a>
                            </div>
